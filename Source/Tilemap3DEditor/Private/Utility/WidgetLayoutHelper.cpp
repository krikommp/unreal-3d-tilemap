...
namespace T3D
{
	void TitleSlot(TSharedPtr<SVerticalBox> VerticalBox, const FText& InLabel, const EVerticalAlignment VAlign)
	{
		VerticalBox->AddSlot()
		           .VAlign(VAlign)
		           .Padding(10.0f, 10.0f)
	...
	SVerticalBox::FScopedWidgetSlotArguments NextHSlot(TSharedPtr<SHorizontalBox> HorizontalBox, const FText& InLabel,
	...
	)
	{
		...
		[
			SNew(SCheckBox)
		.IsChecked_Lambda([HContentBox]
			               {
				               return HContentBox->IsEnabled()
